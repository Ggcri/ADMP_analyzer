module  NUMERIC
  interface RMSD ! GENERIC INTERFACE TO HANDLE SINGLE,DOUPLE AND QUADRUPLE PRECISION MODE
        ...
        subroutine sp_RMSD (rmsd_vec,coord,Step_Num,Natoms,RMSDErrFlag,AT_ids,REF_t)
            use, intrinsic :: iso_fortran_env , only : real32,int8,int16,int8 
            ! DUMMY SECTION 
            real(kind=real32),intent(in), dimension(:) :: coord 
            integer(kind=int16),intent(inout) ::     Step_Num        
            integer(kind=int8),intent(inout) ::     Natoms          
            integer(kind=int8),intent(inout),dimension(Natoms),optional :: AT_ids 
            integer(kind=int16),intent(in),optional ::  REF_t       
        ...
        subroutine qp_RMSD (rmsd_vec,coord,Step_Num,Natoms,RMSDErrFlag,AT_ids,REF_t)
            use, intrinsic :: iso_fortran_env , only : real128,int8,int16,int8 
            ! DUMMY SECTION 
